  ...
  return (
    <div className="p-6 md:p-12 max-w-7xl mx-auto animate-in fade-in duration-1000">
      <header className="mb-16 space-y-3">
        <h2 className="text-5xl md:text-6xl font-black text-slate-900 tracking-tight">Therapy Modules</h2>
        <p className="text-2xl md:text-3xl text-slate-500 font-medium font-sans">Your daily recommended exercises and tasks.</p>
      </header>
      ...
      <section className="glass rounded-[3rem] p-8 md:p-12 shadow-2xl shadow-blue-100 border-none animate-in slide-in-from-bottom-8 duration-700">
        <div className="flex justify-between items-center mb-10 pb-6 border-b border-slate-100">
          <h3 className="text-4xl font-black text-slate-900 tracking-tight">Recommended Therapy</h3>
          <button 
            className="text-blue-700 text-2xl font-black flex items-center gap-3 hover:gap-5 transition-all focus:outline-none focus:ring-4 focus:ring-blue-300 rounded-xl leading-none"
          ...
          {THERAPY_TASKS.map((task, i) => {
            ...
            return (
              <div 
                ...
                className={`relative flex flex-col md:flex-row md:items-center justify-between p-8 rounded-[2rem] border-2 cursor-pointer transition-all duration-500 gap-6 animate-in zoom-in duration-700 ${isCompleted ? 'bg-slate-50/50 border-slate-200 opacity-60 scale-[0.98]' : 'bg-white hover:bg-slate-50 border-white hover:border-slate-200 shadow-sm hover:shadow-xl hover:-translate-y-1'}`} 
                tabIndex="0"
              ...
              >
                <div className="flex flex-col md:flex-row items-start md:items-center gap-8">
                  ...
                  <div className="flex flex-col gap-1">
                    <h4 className={`text-3xl font-black tracking-tight ${isCompleted ? 'text-gray-400 line-through' : 'text-slate-800'}`}>
                      {task.name}
                    </h4>
                ...
                <div className="flex items-center gap-6">
                  {isJustCompleted && (
                    <span className="text-green-800 font-black bg-green-200 px-6 py-3 rounded-2xl animate-pulse shadow-lg shadow-green-100 text-xl border-2 border-green-300">
                      Task Completed! 🎉
                    </span>
                  )}
                  {isCompleted ? (
                    ...
                    <div className="text-2xl text-slate-400 font-black bg-slate-100 px-5 py-2 rounded-xl">{task.duration}</div>
                  )}
                </div>
              </div>
            );
